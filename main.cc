...
void prepare_temp(uint8_t x, uint8_t y, uint8_t t_idx) {
    ...
    memcpy(CHARSET_DEST + (t_idx << 3), CHARSET_DEST + (settlers_map[(y * 40) + x] << 3), 8);
    SCREEN_RAM[(y * 40) + x] = t_idx;
}
void draw_settler(Settler* s) {
    if (s->t1 == 0) s->t1 = assign_char(); if (s->t2 == 0) s->t2 = assign_char();
    ...
    if (s->off_x > 0) {
        ...
        for (uint8_t i = 0; i < 8; i++) { uint16_t row = (uint16_t)src[i] << (8 - s->off_x); dst1[i] |= (uint8_t)(row >> 8); dst2[i] |= (uint8_t)(row & 0xFF); }
    } else if (s->off_y > 0) {
        prepare_temp(s->x, s->y, s->t1); prepare_temp(s->x, s->y + 1, s->t2);
        for (uint8_t i = 0; i < 8; i++) { uint8_t ty = i + s->off_y; if (ty < 8) dst1[ty] |= src[i]; else dst2[ty - 8] |= src[i]; }
    } else { prepare_temp(s->x, s->y, s->t1); for (uint8_t i = 0; i < 8; i++) dst1[i] |= src[i]; }
...
void update_settler(Settler* s) {
    if (s->steps_remaining > 0) {
        ...
        if (--s->steps_remaining == 0) { restore_tile(s->old_x, s->old_y); if (s->dir_x != 0) restore_tile(s->old_x + 1, s->old_y); if (s->dir_y != 0) restore_tile(s->old_x, s->old_y + 1); s->dir_x = 0; s->dir_y = 0; }
    }
}
